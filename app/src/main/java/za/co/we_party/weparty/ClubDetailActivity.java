...
import za.co.we_party.weparty.Adapters.RatingAdapter;
import za.co.we_party.weparty.Models.Club;
import za.co.we_party.weparty.Models.Rating;
...
public class ClubDetailActivity extends AppCompatActivity
    ...
    private Task<Void> addRating(final DocumentReference restaurantRef, final Rating rating) {
        ...
        return mFirestore.runTransaction(new Transaction.Function<Void>() {
            ...
            public Void apply(Transaction transaction)
                ...
                Club club = transaction.get(restaurantRef)
                        .toObject(Club.class);

                // Compute new number of ratings
                int newNumRatings = club.getNumRatings() + 1;

                // Compute new average rating
                double oldRatingTotal = club.getAvgRating() *
                        club.getNumRatings();
                double newAvgRating = (oldRatingTotal + rating.getRating()) /
                        newNumRatings;

                // Set new club info
                club.setNumRatings(newNumRatings);
                club.setAvgRating(newAvgRating);

                // Commit to Firestore
                transaction.set(restaurantRef, club);
    ...
    public void onEvent(DocumentSnapshot snapshot, FirebaseFirestoreException e) {
        ...
        onRestaurantLoaded(snapshot.toObject(Club.class));
    }

    private void onRestaurantLoaded(Club restaurant) {
        ...
        mPriceView.setText(ClubUtil.getPriceString(restaurant));
